...
func (p Page) GetPlainTextV2(fonts map[string]*Font) (result string, err error) {
	defer func() {
		if r := recover(); r != nil {
			result = ""
	...
	for i, t := range texts {
		...
		// New line? We use a threshold of 3 points for Y change.
		if lastY-t.Y > 3 {
			buf.WriteByte('\n')
		} else if t.X > lastX+lastW+1.5 {
			// Same line. Check for a significant gap to insert a space.
			// Threshold is 1.5 points of gap.
			buf.WriteByte(' ')
		}
